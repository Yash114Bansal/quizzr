...
export default function Clientquizzr({ quizzes }: { quizzes: any }) {
    ...
    return <>
        <div className={`flex ${view === "list" ? "flex-col" : "flex-row flex-wrap"} gap-3 w-full mt-4 md:max-h-[60vh] overflow-y-auto`}>
            ...
            {clicked && (
                ...
                >
                    <div className="px-8 py-1 m-1 rounded-md text-dark dark:text-white hover:bg-card-light dark:hover:bg-off-dark cursor-pointer"
                        onClick={() => { router.push(`/admin/quiz/${quizId}`) }}
                    >Open</div>
                    <div className="px-8 py-1 rounded-md text-red-light m-1 hover:bg-card-light dark:hover:bg-off-dark cursor-pointer"
                        onClick={() => setDelModalOpen(true)}
                    >Delete</div>
                </div>
            )}
        </div>
        <ConfirmationModal open={delModalOpen} setOpen={setDelModalOpen} onClick={()=>{deleteQuiz(quizId)}} desc="Are you sure you want to delete this quiz?" />
    </>
}